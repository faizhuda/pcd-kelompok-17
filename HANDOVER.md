...
- GridSearch SVM: `C=[1,10]`, `gamma=['scale', 0.01]`, `cv=3` -> 4 candidates x 3 folds = 12 fits.
...
### [DONE] Sudah diterapkan
1. **Gamma SVM dikembalikan.** `gamma=['scale', 0.01]`, `C=[1,10]`, `cv=3`. 4 candidates x 3 folds = 12 fits - cukup cepat, tuning adil untuk perbandingan CNN vs SVM.
...
4. **Bagian "kelemahan sistem"** di laporan: akui (a) **single split / satu seed** (tak ada CI/repeated runs), (b) **risiko leakage near-duplicate** (foto objek fisik sama bisa tersebar ke train+test -> menggelembungkan semua angka), (c) desain **ladder bukan factorial** (tak bisa lihat interaksi restorasixenhancement).
